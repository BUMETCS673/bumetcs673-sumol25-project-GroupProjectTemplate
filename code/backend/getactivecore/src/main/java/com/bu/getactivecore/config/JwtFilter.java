...
/**
 * A custom JWT filter that intercepts incoming HTTP requests to perform JWT token validation.
 * <p>
...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        /*
        This ensures that only valid JWT tokens are granted access.
        And authentication is only set if not already present, preventing overwriting existing security contexts in a filter chain.
        */
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (m_jwtApi.validateToken(token, userDetails)) {
                // Set the authentication object into the so that downstream filters and controllers can treat the request as authenticated.
